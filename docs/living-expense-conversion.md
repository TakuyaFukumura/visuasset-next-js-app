...
- デフォルト値：0.0%
- 小数点以下1桁まで入力可（例：2.5% を入力した場合は 0.025 として扱う）
- 実利回りは「年率（%）」で入力し、内部的には `実利回り（%） / 100` によって小数に変換してから計算に用いる
- 月初の残高に利回りを反映した後に生活費を支出する（利回り→支出の順で計算）
...
```
当月残高 = 前月残高 × (1 + (実利回り（%） / 100) / 12) - 月次生活費合計
```

- シミュレーション内部の通貨単位はすべて「円」とする
- 残高がゼロ以下になった時点でシミュレーションを終了する
- シミュレーション結果は年次（12ヶ月ごと）に集計してグラフと表に表示する
  - 年次残高は「各年の12ヶ月目終了時点の残高（円）」とする（例：1年目＝12ヶ月目、2年目＝24ヶ月目）
  - 年次残高は内部的には円単位で保持し、表示時にのみ万円換算・丸めを行う
...
- Y軸：資産残高（万円）
  - 年次残高（円）を 10,000 で割って「万円」に換算した値を表示する
  - 丸めルール：円→万円換算後、小数第2位を四捨五入し、小数第1位まで表示する（例：12,356,000円 → 1,235.6万円）
- データ：シミュレーション結果の年次残高（円）を上記ルールで万円換算した値
